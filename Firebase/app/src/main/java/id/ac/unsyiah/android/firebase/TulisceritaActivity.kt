...
import id.ac.unsyiah.android.firebase.databinding.TulisceritaBinding

class TulisceritaActivity : AppCompatActivity() {
    private lateinit var mAuth: FirebaseAuth
    private lateinit var database: DatabaseReference

    private var _binding: TulisceritaBinding? = null
    private val binding get() = _binding!!
    private lateinit var progressDialog: ProgressDialog

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = TulisceritaBinding.inflate(layoutInflater)
        ...
        database.child("USERS").child(user.uid).addListenerForSingleValueEvent(object :
            ...
            override fun onDataChange(data: DataSnapshot) {
                if (data.exists()) {
                    ...
                    startActivity(Intent(this@TulisceritaActivity, LoginActivity::class.java))
                    finish()
                }
    ...
    private fun setupProfile(userMC: UserMC) {
        binding.post.setText(userMC.story)

        binding.sendstory.setOnClickListener {
            val userReff = database.child("USERS").child(mAuth.uid!!)
            userReff.child("story").setValue(binding.post.text.toString())

            userReff.child(userMC.story).setValue(userMC).addOnCompleteListener {
                if(it.isSuccessful) {
